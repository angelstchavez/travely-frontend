...
const CustomerCompanyReportBox = () => {
  ...
  const getCustomerCount = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/customers/count/companies`,
        {
          method: "GET",
          headers: {
            accept: "*/*",
  ...
  return (
    <BoxReport
      cantidad={countData}
      icon={
        <RiCommunityLine className="text-2xl bg-pink-500/10 text-pink-500 p-2 box-content rounded-xl" />
      }
      title={
        <h1 className="text-pink-500 text-2xl font-semibold">Empresariales</h1>
      }
    />
  );
};

export default CustomerCompanyReportBox;

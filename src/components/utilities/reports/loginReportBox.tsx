...
const loginReportBox = () => {
  ...
  const getCustomerCount = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/logins/active/count`,
        {
          method: "GET",
          headers: {
            accept: "*/*",
  ...
  return (
    <BoxReport
      ...
      title={<h1 className="text-red-500 text-2xl font-semibold">Inicios de sesión</h1>}
    />
  );
};

export default loginReportBox;

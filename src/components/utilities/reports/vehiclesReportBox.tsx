...
const vehicleReportBox = () => {
  ...
  const getCustomerCount = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/vehicles/active/count`,
        {
          method: "GET",
          headers: {
            accept: "*/*",
  ...
  return (
    <BoxReport
      cantidad={countData}
      icon={
        <RiCarLine className="text-2xl bg-sky-500/10 text-sky-500 p-2 box-content rounded-xl" />
      }
      title={
        <h1 className="text-sky-500 text-2xl font-semibold">
          Vehiculos activos
        </h1>
      }
    />
...
export default vehicleReportBox;
